...
(function () {
    ...
    var mapDiv;
    var trafficLayer;
    var hfdStationsLayer;
    var hfdStationsLayerVisible;
    var hfdHydrantsLayer;
    var hfdHydrantsLayerVisible;
    var hfdDistrictsLayer;
    var hfdDistrictsLayerVisible;
    var infoWindow;
    var incidents;
    var markers;
    var jqMapDiv = null;
    ...
    function initialize() {
        ...
        trafficLayer = null;
        hfdStationsLayer = null;
        hfdHydrantsLayer = null;
        hfdDistrictsLayer = null;
        hfdStationsLayerVisible = false;
        hfdHydrantsLayerVisible = false;
        hfdDistrictsLayerVisible = false;
        ...
        if (hfdDistrictsLayer === null) {
            hfdDistrictsLayer = new google.maps.FusionTablesLayer({
                map: map,
                heatmap: { enabled: false },
                query: {
                    select: "geometry",
                    from: "1eG0w9kLrF7je9tTCt--FM-FymI7dBH6qjrKxT8FL",
                    where: ""
                },
                options: {
                    styleId: 2,
            ...
            hfdDistrictsLayerVisible = true;

        }

        if (hfdStationsLayer === null) {
            hfdStationsLayer = new google.maps.FusionTablesLayer({
                map: map,
                heatmap: { enabled: false },
                query: {
                    select: "geometry",
                    from: "1MErXqdyKuMjQLix3-EEEZeLrxYRTMcT6YFI-yCqX",
                    where: ""
                },
                options: {
                    styleId: 2,
            ...
            hfdStationsLayerVisible = true;

        }

        if (hfdHydrantsLayer === null) {
            hfdHydrantsLayer = new google.maps.FusionTablesLayer({
                heatmap: { enabled: false },
                query: {
                    select: "LAT",
                    from: "1ed5-Ne_q8Xyo1KHdDc1s7hPvC_OtFyTxOoC6kgQ7",
                    where: ""
                },
                options: {
                    styleId: 2,
        ...
        map.addListener('zoom_changed', function () {
            ...
            if (currentZoom >= 11) {
                if (hfdStationsLayerVisible === false) {
                    hfdStationsLayer.setMap(map);
                    hfdStationsLayerVisible = true;
                }
                if (hfdDistrictsLayerVisible === false) {
                    hfdDistrictsLayer.setMap(map);
                    hfdDistrictsLayerVisible = true;
                }

            } else {
                if (hfdStationsLayerVisible === true) {
                    hfdStationsLayer.setMap(null);
                    hfdStationsLayerVisible = false;
                }
                if (hfdDistrictsLayerVisible === true) {
                    hfdDistrictsLayerVisible = false;
                    hfdDistrictsLayer.setMap(null);
                }
            }

            if (currentZoom >= 16) {
                if (hfdHydrantsLayerVisible === false) {
                    hfdHydrantsLayer.setMap(map);
                    hfdHydrantsLayerVisible = true;
                }
                if (hfdDistrictsLayerVisible === true) {
                    hfdDistrictsLayerVisible = false;
                    hfdDistrictsLayer.setMap(null);
                ...
                if (hfdHydrantsLayerVisible === true) {
                    hfdHydrantsLayer.setMap(null);
                    hfdHydrantsLayerVisible = false;
                }
            }
        });
